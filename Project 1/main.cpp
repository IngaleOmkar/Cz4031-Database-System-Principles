...
void experiment_2(BPTree root)
{
     ...
     cout << "Number of Nodes in B+ Tree: "
          << root.calculateNodeNum(root.getRoot()) << "\n";
     cout << "B+ Tree Height: " << root.getHeight(root.getRoot()) << "\n";

     cout << "\nB+ Tree:\n";

     root.show(root.getRoot());
}
...
void experiment_5(BPTree root, int numVotes)
{
     ...
     int numNodeBeforeRemoval = root.calculateNodeNum(root.getRoot());
     int numNodeMerged = root.remove(key);
     int numNodeAfterRemoval = root.calculateNodeNum(root.getRoot());
